#####################################################
### 22. Cook Inlet Fiberoptic Network,	ak_cs_027 ###
#####################################################
...
for(i in 1:length(code_dict$study_area)){
  ...
  ## for each item in the code_dict list, search the study area geopackage for the hex layer corresponding to that item
  # buffer by 500 m (this is computationally faster than buffering the dataset first and then intersecting)
  hex_by_study <- sf::st_read(dsn = hex_path,
                              layer = sf::st_layers(hex_path)[[1]][grep(pattern = dataset,
                                                                        sf::st_layers(dsn = hex_path)[[1]])]) %>% sf::st_buffer(dist = 500)
  ...
  clean_data <- function(raw_data){
    data_layer <- raw_data %>%
      ...
      # add a setback (buffer) distance of 500 m
      sf::st_buffer(dist = 500) 
    return(data_layer)
  }
  
  # Load Cook Inlet Fiberoptic Network and pass it through the cleaning function above
  ## source: https://data.axds.co/gs/nfwf/wfs?service=WFS&version=1.0.0&request=GetFeature&outputFormat=SHAPE-ZIP&typeName=nfwf:ci_fiberoptic
  ## metadata: https://portal.aoos.org/?portal_id=121#metadata/90f03198-8cd2-11e5-b1ac-00265529168c/a7a9787c-8cd2-11e5-b1b0-00265529168c
  ### data
