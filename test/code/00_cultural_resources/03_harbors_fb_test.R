...
for(i in code_dict$study_area){
  ...
  tryCatch({
    ...
    # layer 
    hex_layer = paste(code_dict$code[i], "cs_fb_suitability_unconstrained", sep = "_")

    ## for each item in the code_dict list, search the study area geopackage for the dataset layer corresponding to that item
    hex_by_study <- sf::st_read(dsn = hex_path, layer = hex_layer)
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "harbors" for summary
        dplyr::mutate(layer = "harbors")
      return(data_layer)
    }
