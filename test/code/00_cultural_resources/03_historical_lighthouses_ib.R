...
for(i in 1:length(code_dict$study_area)){
  ...
  tryCatch({
    ...
    hex_path <- file.path(study_area_dir, code_dict$study_area[i], "1_intertidal/d_suitability_data/constraints/2_constraints.gpkg")
    
    # layer 
    layer = paste(code_dict$code[i], "cs_ib_suitability_unconstrained", sep = "_")
