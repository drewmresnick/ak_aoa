########################################################
### 19. Anadromous Water Catalog: Stream,	ak_cs_024  ###
########################################################
...
for(i in 1:length(code_dict$study_area)){
  ...
  # buffer by 91.5m (this is computationally faster than buffering the dataset first and then intersecting)
  hex_by_study <- sf::st_read(dsn = hex_path,
                              layer = sf::st_layers(hex_path)[[1]][grep(pattern = dataset,
                                                        sf::st_layers(dsn = hex_path)[[1]])]) %>% sf::st_buffer(dist = 91.5)
  ...
  clean_data <- function(raw_data){
    data_layer <- raw_data %>%
      ...
      # add a setback (buffer) distance of 91.5m
      sf::st_buffer(dist = 91.5) 
    return(data_layer)
  }
  
  # Load Anadromous Water Catalog: Stream and pass it through the cleaning function above
  ## source: https://www.adfg.alaska.gov/static-sf/AWC/GDB/2023GDB_statewide.zip
  ## metadata: https://www.adfg.alaska.gov/sf/SARR/AWC/index.cfm?ADFG=maps.dataFiles 
  ### data
  ...
  # new unique code that's specific to the study area, not just the region
