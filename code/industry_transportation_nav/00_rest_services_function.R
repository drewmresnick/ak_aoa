#####################################################################################
### 0. Download Data -- Industry, transportation, navigation REST server download ###
#####################################################################################
...
# Permitted Log Transfer Facilities Public View

url_list <- c(
  "services/Permitted_Log_Transfer_Facilities_Public_View/FeatureServer/0"
)
...
work <- parallel::parLapply(cl = cl, X = url_list, fun = rest_services_function, base_url = "https://services1.arcgis.com/7HDiw78fcUiM2BWn/arcgis/rest/", data_dir = data_dir)

parallel::stopCluster(cl = cl)


# Rename the Permitted_Log_Transfer_Facilities_Public_View to log_transfer_facilities for a more descriptive title
file.rename(from = file.path(data_dir, list.files(data_dir,
                                                  # get the harbors directory
                                                  pattern = "Permitted_Log_Transfer_Facilities_Public_View")),
            to = file.path(data_dir, "log_transfer_facilities"))
