############################################################################################################
### 28. Alaska DEC Seafood Processing Facilities: Seafood Processing Facility Locations,	ak_cs_034      ###
############################################################################################################
...
for(i in 1:length(code_dict$study_area)){
  ...
  clean_data <- function(raw_data){
    data_layer <- raw_data %>%
      ...
      # add a setback (buffer) distance of 500m
      sf::st_buffer(dist = 500) 
    return(data_layer)
  }
  
  # Load Alaska DEC Seafood Processing Facilities: Seafood Processing Facility Locations and pass it through the cleaning function above
  ## source: https://gis.data.alaska.gov/datasets/ADEC::alaska-dec-seafood-processing-facilities/explore?layer=2&location=7.769055%2C0.000000%2C1.71
  ### data
