####################################################################
### 18. ADF&G Active Aquatic Farming Operation Areas, ak_cs_019  ###
####################################################################
  ...
  tryCatch({
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        dplyr::filter(permit_ty0 %in% c("Aquatic Farm Permit")) %>%
        # obtain data within buffered hexed study area
        rmapshaper::ms_clip(hex_by_study) %>%
        # create field called "layer" and fill with "active aquatic farming op areas" for summary
        dplyr::mutate(layer = "active aquatic farming op areas")
      return(data_layer)
    }
    
    # Load Active Aquatic Farming Operation Areas and pass it through the cleaning function above
