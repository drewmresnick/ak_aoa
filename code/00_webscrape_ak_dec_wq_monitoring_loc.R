######################################################################################
### 0. Webscrape Data -- Alaska Department of Environmental Conservation no shadow ###
######################################################################################
...
# Navigates to Alaska DEC WG Monitoring locations (source: https://data-soa-adec.opendata.arcgis.com/datasets/ADEC::alaska-dec-wq-monitoring-locations/about)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://data-soa-adec.opendata.arcgis.com/datasets/ADEC::alaska-dec-wq-monitoring-locations/about"
...
file.rename(from=file.path(download_dir, "Alaska_DEC_WQ_Monitoring_Locations.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "Alaska_DEC_WQ_Monitoring_Locations.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "Alaska_DEC_WQ_Monitoring_Locations")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "Alaska_DEC_WQ_Monitoring_Locations.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "Alaska_DEC_WQ_Monitoring_Locations.zip"))
