########################################################################################################
### 25. Alaska DEC Seafood Processing Facilities: AKG130000 Permitted Outfall,	ak_cs_031            ###
########################################################################################################
  ...
  tryCatch({
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "seafood processing permited outfall" for summary
        dplyr::mutate(layer = "seafood processing permited outfall") %>%
        # add a setback (buffer) distance of 500m
        sf::st_buffer(dist = 500) 
      return(data_layer)
    }
    
    # Load Alaska DEC Seafood Processing Facilities: AKG130000 Permitted Outfall and pass it through the cleaning function above
    ## REST Service: https://dec.alaska.gov/arcgis/rest/services/Water/Seafood_Processing/MapServer/12
    ### data
