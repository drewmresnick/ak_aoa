##################################################################
### 30. Alaska DEC WQ Monitoring Locations	ak_cs_036          ###
################################################################## 
  ...
  tryCatch({
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "wq monitoring locations" for summary
        dplyr::mutate(layer = "wq monitoring locations")
      return(data_layer)
    }
    
    # Load Alaska DEC WQ Monitoring Locations and pass it through the cleaning function above
    ## source: https://data-soa-adec.opendata.arcgis.com/datasets/ADEC::alaska-dec-wq-monitoring-locations/about
    ### data
