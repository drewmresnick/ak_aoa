...
for(i in 1:length(code_dict$study_area)){
  ...
  tryCatch({
    ...
    # buffer by 350m (this is computationally faster than buffering the dataset first and then intersecting)
    hex_by_study <- sf::st_read(dsn = hex_path,
                                layer = sf::st_layers(hex_path)[[1]][grep(pattern = dataset,
                                                                          sf::st_layers(dsn = hex_path)[[1]])])%>% sf::st_buffer(dist = 350)
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "ferry terminals" for summary
        dplyr::mutate(layer = "ferry terminals") %>%
        # add a setback (buffer) distance of 350m
        sf::st_buffer(dist = 350) 
      return(data_layer)
    }
    
    # Load Ferry Terminals and pass it through the cleaning function above
    ## source: https://data-soa-akdot.opendata.arcgis.com/datasets/AKDOT::ferry-terminals-2/explore
    ### data
