#######################################################################
### 14. Wastewater Outfalls: Wastewater Outfall Pipes,	ak_cs_015   ###
#######################################################################
  ...
  tryCatch({
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "wastewater outfall pipes" for summary
        dplyr::mutate(layer = "wastewater outfall pipes") %>%
        # add a setback (buffer) distance of 500m
        sf::st_buffer(dist = 500) 
      return(data_layer)
    }
    
    # Load Wastewater Outfalls and pass it through the cleaning function above
    ## source: https://marinecadastre.gov/downloads/data/mc/WastewaterOutfall.zip
