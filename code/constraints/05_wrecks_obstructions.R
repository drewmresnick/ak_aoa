#############################################################
### 5. Wrecks and Obstructions , ak_cs_003                ###
#############################################################
...
for(i in 1:length(code_dict$study_area)){
  ...
  tryCatch({
    ...
    # buffer by 152.4m (this is computationally faster than buffering the dataset first and then intersecting)
    hex_by_study <- sf::st_read(dsn = hex_path,
                                layer = sf::st_layers(hex_path)[[1]][grep(pattern = dataset,
                                                                          sf::st_layers(dsn = hex_path)[[1]])]) %>% sf::st_buffer(dist = 152.4)
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "wrecks and obstructions" for summary
        dplyr::mutate(layer = "wrecks and obstructions") %>%
        # add a setback (buffer) distance of 152.4m
        sf::st_buffer(dist = 152.4) 
      return(data_layer)
    }
    
    # Load Wrecks and Obstructions and pass it through the cleaning function above
    ## source: https://marinecadastre.gov/downloads/data/mc/WreckObstruction.zip
