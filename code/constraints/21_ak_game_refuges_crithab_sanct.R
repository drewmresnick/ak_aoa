################################################################################
### 21. Alaska State Game Refuges, Critical Habitat, Sanctuaries,	ak_cs_026  ###
################################################################################
  ...
  tryCatch({
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "game refuges crithab sanct" for summary
        dplyr::mutate(layer = "game refuges crithab sanct")
      return(data_layer)
    }
    
    
    # Load Alaska State Game Refuges, Critical Habitat, Sanctuaries and pass it through the cleaning function above
    ## source: https://data-soa-adec.opendata.arcgis.com/datasets/ADEC::alaska-dec-sensitive-areas/explore?layer=16&location=50.000528%2C-154.002228%2C20.60
    ### data
