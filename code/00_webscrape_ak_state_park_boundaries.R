#########################################################################################################
### 0. Webscrape Data -- Alaska State Parks Boundary shadow                                           ###
#########################################################################################################
...
# Navigates to Alaska State Parks (source: https://gis.data.alaska.gov/datasets/SOA-DNR::state-park-boundary/explore)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://gis.data.alaska.gov/datasets/SOA-DNR::state-park-boundary/explore"
...
file.rename(from=file.path(download_dir, "State_Park_Boundary.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "ak_state_park_boundary.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "ak_state_park_boundary.zip")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "ak_state_park_boundary.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "ak_state_park_boundary.zip"))
