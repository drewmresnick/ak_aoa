#############################################################
### 8. Submarine Cable Areas, ak_cs_006                   ###
#############################################################
  ...
  tryCatch({
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "submarine cable areas" for summary
        dplyr::mutate(layer = "submarine cable areas") %>%
        # add a setback (buffer) distance of 200m
        sf::st_buffer(dist = 200) 
      return(data_layer)
    }
    
    # Load Submarine Cable Areas and pass it through the cleaning function above
    ## source: https://marinecadastre.gov/downloads/data/mc/SubmarineCableArea.zip 
