##########################################
### 20. Alaska State Parks,	ak_cs_025  ###
##########################################
...
for(i in 1:length(code_dict$study_area)){
  ...
  tryCatch({
    ...
    # buffer by 100m (this is computationally faster than buffering the dataset first and then intersecting)
    hex_by_study <- sf::st_read(dsn = hex_path,
                                layer = sf::st_layers(hex_path)[[1]][grep(pattern = dataset,
                                                                          sf::st_layers(dsn = hex_path)[[1]])])%>% sf::st_buffer(dist = 100)
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "alaska state parks" for summary
        dplyr::mutate(layer = "alaska state parks") %>%
        # add a setback (buffer) distance of 100m
        sf::st_buffer(dist = 100) 
      return(data_layer)
    }
    
    # Load Alaska State Parks and pass it through the cleaning function above
    ## source: https://gis.data.alaska.gov/datasets/SOA-DNR::state-park-boundary/explore
    ### data
