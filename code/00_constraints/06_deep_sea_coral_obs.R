#############################################################
### 6. Deep Sea Coral Observations , ak_cs_004            ###
#############################################################
...
for(i in 1:length(code_dict$study_area)){
  ...
  tryCatch({
    ...
    # buffer by 1000m (this is computationally faster than buffering the dataset first and then intersecting)
    hex_by_study <- sf::st_read(dsn = hex_path,
                                layer = sf::st_layers(hex_path)[[1]][grep(pattern = dataset,
                                                                          sf::st_layers(dsn = hex_path)[[1]])]) %>% sf::st_buffer(dist = 1000)
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "deep_sea_coral_obs" for summary
        dplyr::mutate(layer = "deep_sea_coral_obs") %>%
        # add a setback (buffer) distance of 1000m
        sf::st_buffer(dist = 1000) 
      return(data_layer)
    }
    
    # Load Deep Sea Coral Observations and pass it through the cleaning function above
