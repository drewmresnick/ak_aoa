#############################################################
### 9. Environmental Sensors and Buoys, ak_cs_007         ###
#############################################################
...
for(i in 1:length(code_dict$study_area)){
  ...
  tryCatch({
    ...
    # buffer by 500m (this is computationally faster than buffering the dataset first and then intersecting)
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "sensors and buoys" for summary
        dplyr::mutate(layer = "sensors and buoys") %>%
        # add a setback (buffer) distance of 500m
        sf::st_buffer(dist = 500) 
      return(data_layer)
    }
    
    # Load Environmental Sensors and Buoys and pass it through the cleaning function above
    ## source: https://www.ndbc.noaa.gov/kml/marineobs_by_pgm.kml 
    ## interactive map: https://www.ndbc.noaa.gov/obs.shtml 
    ### data
