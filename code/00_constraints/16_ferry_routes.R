  ...
  tryCatch({
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "ferry routes" for summary
        dplyr::mutate(layer = "ferry routes") %>%
        # add a setback (buffer) distance of 500m
        sf::st_buffer(dist = 500) 
      return(data_layer)
    }
    
    # Load Ferry Routes and pass it through the cleaning function above
    ## source: https://data-usdot.opendata.arcgis.com/datasets/usdot::ferry-routes/about
    ### data
