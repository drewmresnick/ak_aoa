#####################################################
### 17. Aquatic Farm Permit or Lease,	ak_cs_018   ###
#####################################################
...
for(i in 1:length(code_dict$study_area)){
  ...
  tryCatch({
    ...
    # buffer by 50m (this is computationally faster than buffering the dataset first and then intersecting)
    hex_by_study <- sf::st_read(dsn = hex_path,
                                layer = sf::st_layers(hex_path)[[1]][grep(pattern = dataset,
                                                                          sf::st_layers(dsn = hex_path)[[1]])])%>% sf::st_buffer(dist = 50)
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "aquatic farm permit lease" for summary
        dplyr::mutate(layer = "aquatic farm permit lease") %>%
        # add a setback (buffer) distance of 50m
        sf::st_buffer(dist = 50) 
      return(data_layer)
    }
    
    
    # Load Aquatic Farm Permit or Lease and pass it through the cleaning function above
    ## source: https://data-soa-dnr.opendata.arcgis.com/datasets/SOA-DNR::aquatic-farm-permit-or-lease-polygon/explore 
    ### data
