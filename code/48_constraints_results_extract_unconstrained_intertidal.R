###################################################################################
### 48. Constraint: extracting unconstrained intertidal scenario hexes          ###
###################################################################################
...
layer <- c("constraints_intertidal")
...
for(i in 1:length(code_dict$study_area)){
  ...
  clean_data <- function(constraints){
    data_layer <- constraints %>%
      ...
      dplyr::filter(constr_intertidal == "1") 
    return(data_layer)
  }
  ...
  suitability_output_gpkg <- file.path("C:/Users/Breanna.Xiong/Documents/R Scripts/ak_aoa/study_area", code_dict$study_area[i], "1_intertidal/d_suitability_data/constraints/constraints_suitability.gpkg")
  
  # Create the new layer name based on the study area
  new_layer_name <- paste("ak", code_dict$code[i], "unconstrained_intertidal", sep = "_")
