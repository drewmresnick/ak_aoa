########################################################################################################
### 35. Land Status within the National Wildlife Refuges of Alaska: Boundaries Refuge	ak_cs_043      ###
########################################################################################################
  ...
  tryCatch({
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        dplyr::filter(ISLAND_NAME == "Afognak Island" | SUB_UNIT == "Womens Bay") %>%
        # obtain data within buffered hexed study area
        rmapshaper::ms_clip(hex_by_study) %>%
        # create field called "layer" and fill with "land status refuge" for summary
        dplyr::mutate(layer = "land status refuge")
      return(data_layer)
    }
    
    # Load Land Status within the National Wildlife Refuges of Alaska: Boundaries and pass it through the cleaning function above
    ## source: https://www.fws.gov/r7/nwr/Realty/data/GIS/Land-Status/USFWS-Region-7-Land-Status.gdb.zip
    ## arcgis online: https://www.arcgis.com/home/item.html?id=3eed8d6b30ea443dafe4380d70d0fa5e
    ### data
