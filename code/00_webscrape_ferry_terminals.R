#############################################################################
### 0. Webscrape Data -- Ferry Terminals no shadow                        ###
#############################################################################
...
# Navigates to Alaska Department of Transportation Ferry terminals (source: https://data-soa-akdot.opendata.arcgis.com/datasets/AKDOT::ferry-terminals-2/explore)
# and scrapes the site data for the states and regions of interest

# Base URL
base_url <- "https://data-soa-akdot.opendata.arcgis.com/datasets/AKDOT::ferry-terminals-2/explore"
...
####          is for the CSV, hence the 3rd element is for the shapefile
...
file.rename(from=file.path(download_dir, "Ferry_Terminals_-2373859628527675968.zip"),  # Make default download directory flexible
            # send to the raw data directory
            to=file.path(data_dir, "Ferry_Terminals_-2373859628527675968.zip"))

#####################################

# Unzip the data
## grab text before ".zip" and keep only text before that
new_dir_name <- sub(".zip", "", "ferry_terminals")

## create new directory for data
new_dir <- file.path(data_dir, new_dir_name)

## unzip the file
unzip(zipfile = file.path(data_dir, "ferry_terminals.zip"),
      # export file to the new data directory
      exdir = new_dir)

## remove original zipped file
file.remove(file.path(data_dir, "ferry_terminals.zip"))
